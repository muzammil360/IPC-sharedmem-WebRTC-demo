...
static gchar* window_id_str = NULL;
static guint64 window_id = 0; 
static xdo_t * xdo = NULL;

char * pSharedMem = NULL;   // MUZ: 

static GOptionEntry entries[] = {
  {"window-id", 0, 0, G_OPTION_ARG_STRING, &window_id_str,
      "WINDOW ID of the 3D APP", "ID"},  
...
{
  xdo_activate_window(xdo, (Window) window_id);
  xdo_enter_text_window(xdo, (Window) window_id, str, 0);
...
start_pipeline (gboolean create_offer)
{
  ...
  //     STUN_SERVER
  //     "ximagesrc xid=%s"
  //     " ! video/x-raw,framerate=5/1 ! videoconvert ! queue ! "
  //     "vp8enc deadline=1 keyframe-max-dist=2000 ! "
  //     /* picture-id-mode=15-bit seems to make TWCC stats behave better */
  //     "rtpvp8pay name=videopay picture-id-mode=15-bit ! "
  //     "queue ! " RTP_CAPS_VP8 "96 ! sendrecv. ", window_id_str);



  pipe1 = gst_parse_launch (pipeline_str, &error);
  g_free(pipeline_str);
  if (error) {
...
main (int argc, char *argv[])
{
    ...
    size_t sharedMemSizeBytes = 800*600*3; 
    int shmIdOut = SHM_ID_DEFAULT;     // we only need it here for func signation  


    pSharedMem = sharedMemInit(filename, sharedMemSizeBytes, &shmIdOut);
    if (shmIdOut == SHM_ID_DEFAULT) {
          gst_printerr("error: failed to open shared memory\n");
  ...
  if (!window_id_str) {
    gst_printerr ("--window-id is a required argument\n");
    goto out;
  }
  
  xdo = xdo_new(NULL);
  window_id = g_ascii_strtoull(window_id_str, NULL, 10);
